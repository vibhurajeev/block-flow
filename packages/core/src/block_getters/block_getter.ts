import EthClass, { Block, BlockTransactionObject, Eth, TransactionReceipt } from "web3-eth";
...
export class BlockGetter extends BlockFormatter implements IBlockGetter {
     ...
     * @static
     * 
     * Factory method to create block getter instance.
     * 
     * @param {string} wssEndpoint - The websocket endpoint url
     * @param {number} maxRetries - The maximum number times to retry on failure.
     * 
     * @returns {BlockGetter} - Instance of block getter. 
     */
    public static new(wssEndpoint: string, maxRetries: number = 0): BlockGetter {
